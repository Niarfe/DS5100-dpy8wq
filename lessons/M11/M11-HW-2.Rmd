...
# Student Info
    ...
    Net ID: dpy8wq
    File GitHub URL: https://github.com/Niarfe/DS5100-dpy8wq/blob/main/lessons/M11/M11-HW-2.Rmd
...
```{r}
# CODE HERE
Data <- read.csv('./data/abalone.data', header=FALSE)
head(Data)
```
...
groups <- Data %>%
  group_by(V1) %>% # SPLIT
  summarize(mean_shell = mean(V2)) # APPLY
groups  # COMBINE
```
...
```{r}
# CODE HERE
male_median_shell <- Data %>%
  filter(V1 == 'M') %>% # SPLIT
  summarize(median_shell = median(V2)) # APPLY
male_median_shell  # COMBINE
```
